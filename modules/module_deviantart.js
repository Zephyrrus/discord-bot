...
function deviantGrab(e, args) {
    ...
    var name = encodeURIComponent(args);
    var offset = Math.floor(Math.random() * 50);

    e.bot.sendMessage({
      to: e.channelID,
      message: `<@${e.userID}> I am searching on deviantArt for **${args}**`
      ...
      getList(name, offset, function(err, o) {
          ...
          if(o.list && o.list.length != 0) {
            ...
            e.bot.editMessage({
              channel: response.channel_id,
              messageID: response.id,
              message: `<@${e.userID}> Here is a picture of **${args}**:\n**Permalink:** \"${o.list[random].permalink}\"\n\n${o.list[random].url}`
